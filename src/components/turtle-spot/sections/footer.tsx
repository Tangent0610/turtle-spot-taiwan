...
export const Footer = () => (
  <footer className="h-[420px] overflow-hidden bg-turtle-cyan px-20 py-14 text-turtle-ink max-lg:h-[392px] max-lg:px-6 max-lg:py-9 max-sm:px-4">
    <div className="mx-auto grid max-w-[1280px] grid-cols-[1fr_220px_220px] gap-20 max-lg:block">
      ...
      <div className="pt-12 text-sm font-black leading-6 max-lg:mt-14 max-lg:pt-0 max-lg:text-left max-lg:text-lg max-lg:leading-8">
        <p className="max-lg:hidden">{footerContent.contactLabel}</p>
        {footerContent.links.map((link, index) => (
          <a
            className={`${footerLinkClass} ${index > 0 ? "max-lg:mt-3" : ""}`}
            href={link.href}
            key={link.label}
          >
            {link.label}
